...
function changeDims(val, width, height) {
    console.log(val)
    var temp=val.substring(val.indexOf('src'),val.indexOf('"',val.indexOf('src')+6));
    temp='<img ' + temp + '\" width=' + width + ' height='
        ...
        .then(data => {
            ...
            fetch(post).then(dat => dat.json())
                .then(res => {
                    ...
                    arr[0] = arr[0].substring(val, val2 + 2).replace("//", "https://")
                    //resize optional, but we'll do it why not
                    arr[0] = changeDims(arr[0], 240, 240)
